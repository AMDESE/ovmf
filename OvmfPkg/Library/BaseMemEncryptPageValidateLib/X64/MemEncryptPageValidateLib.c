...
#define RMP_PAGES_COUNT(x, y)     (((y == PVALIDATE_PAGE_SIZE_4K) ? \
                                    x : x * PAGES_PER_2MB))
  ...
  )
{
  ...
  while (Start < End) {
    ...
    Return = AsmPvalidate (RmpPageSize, Validate, Start, &EFlags);
    ...
    // a 4K.
    //
    if ((Return == PVALIDATE_RET_FAIL_SIZEMISMATCH) && IS_LARGE_PAGE(Start)) {
      Return = PvalidateRange (Start, PAGES_PER_2MB,
                    PVALIDATE_PAGE_SIZE_4K, Validate);
    }
  ...
  )
{
  ...
  for (i = 0; NumOfPages && i < MaxNumEntries; i++) {
    ...
    if (0 && (NumOfPages >= PAGES_PER_2MB)) {
      //
      // If remaining number of pages is greater than 512 and address is
      // a aligned to a large page then request a 2MB RMP entry.
      //
      if (IS_LARGE_PAGE(Start)) {
        Info->Data.RmpPageSize = PVALIDATE_PAGE_SIZE_2M;

        //
        // Calculate the maximum number of large pages we can process in one
        // go.
        //
        Info->Data.NumOfPages =
          MIN (NumOfPages / PAGES_PER_2MB, MEM_OP_MAX_NPAGES);
      } else {
        //
        // The number of pages are greater than 512 but the start address is
        // not 2MB aligned. Request to add the address range as a 4K.
        //
        Info->Data.RmpPageSize = PVALIDATE_PAGE_SIZE_4K;

        //
        // Round up to the next 2MB address and calculate the minimum number
        // of pages that need to be added as a 4K in the RMP table.
        //
        Pages = ROUND_UP_2M(Info->Data.GuestFrameNumber) -
                            Info->Data.GuestFrameNumber;
    ...
    NumOfPages -=
      RMP_PAGES_COUNT(Info->Data.NumOfPages, Info->Data.RmpPageSize);
  ...
  )
{
  ...
  //
  // If we don't have a valid GHCB then RMPUPDATEs are forced to 4K page size
  // so all the validation will happen as a 4K.
  //
  Msr.GhcbPhysicalAddress = AsmReadMsr64 (MSR_SEV_ES_GHCB);
  if ((Msr.GhcbPhysicalAddress & GHCB_INFO_MASK) != 0) {
    return PvalidateRange (Start, NumOfPages, PVALIDATE_PAGE_SIZE_4K,
                           Validate);
  }
  ...
  while (NumOfPages) {
    ...
    for (i = 0; i < Hdr->Data.NumElements; i++, Info++) {
      Status = PvalidateRange (Info->Data.GuestFrameNumber << EFI_PAGE_SHIFT,
                              Info->Data.NumOfPages,
  ...
  )
{
  ...
  while (Start < End) {
    ...
    Msr.GhcbSnpMemOp.RmpPageSize = PVALIDATE_PAGE_SIZE_4K; // hardcode to 4K
